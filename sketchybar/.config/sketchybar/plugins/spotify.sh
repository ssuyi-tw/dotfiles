...
hide_spotify() {
  # The other spotify.* items live inside popup.spotify.anchor, so they're
  # only visible while the popup is open — hiding the anchor is enough.
  sketchybar -m --set spotify.anchor drawing=off popup.drawing=off
}

# `pgrep -x` reads the process list directly — unlike `tell application "Spotify"`,
# it never goes through Launch Services / the startuphelper mach port, so it cannot
# revive a quitting Spotify.
spotify_is_running() {
  pgrep -xq Spotify
}

update() {
  # music_change fires while Spotify is still mid-quit, so a single check can
  # race and see it alive. Wait briefly and re-check before talking to it.
